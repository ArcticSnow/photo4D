...
class Photo4d(object):
    ...
    def detect_GCPs(self, kernel_size=(200, 200), display=True, save_df_gcp=True):
        '''
        Function to automatically extract GCP location in all stacks of images. This Function can take a while as it search for every GCP their location in every image.
        From the picked GCPs on the reference images, SIFT tries detecting if there was any movement in between the reference image, and any other image from the stack
        '''
